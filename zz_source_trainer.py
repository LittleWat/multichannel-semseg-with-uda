...
parser.add_argument('--train_img_shape', default=(1024, 512), nargs=2, metavar=("W", "H"),
                    help="W H")
parser.add_argument('--net', type=str, default="fcn", choices=['fcn', 'psp', 'segnet'],
...
if args.net == "fcn":
    ...
    model = torch.nn.DataParallel(ResFCN(num_class, args.res, args.input_ch))
else:
...
if args.resume:
    ...
    print ("Trained models are loaded!")
...
for epoch in range(epoches):
    ...
    model_fn = os.path.join(pth_dir, "%s-%s-%s-%d.pth" % (args.savename, args.net, args.res, epoch + 1))
