...
from models.model_util import fix_batchnorm_when_training, get_optimizer, fix_dropout_when_training, \
    get_triple_multitask_models, get_segbd_multitask_models
...
if args.resume:
    ...
    # -------------------------------------- #
    args.epochs = epochs
    model_enc, model_dec = get_segbd_multitask_models(net_name=args.net, input_ch=args.input_ch,
...
mode = "%s-%s2%s-%s_%sch_MCD_segbd_multitask" % (
...
for epoch in range(start_epoch, args.epochs):
    ...
    for ind, (source, target) in tqdm.tqdm(enumerate(train_loader)):
        ...
        src_semseg_loss, src_boundary_loss = model_dec.get_loss(src_fet, src_gt_semseg, separately_returning=True)
        ...
        loss = src_semseg_loss + src_boundary_loss + tgt_psuedo_boundary_loss
    ...
    print("SrcSemsegLoss: %.4f, SrcBoundaryLoss: %.4f  TgtPsuedoBoundaryLoss: %.4f" %
          (src_semseg_loss_per_epoch, src_boundary_loss_per_epoch, tgt_psuedo_boundary_loss_per_epoch))
