...
from torchvision.transforms import ToTensor, Compose, Normalize, Scale
...
def unnormalize(np_input_img, normalize_way="imagenet"):
    if normalize_way == "imagenet":
        std = np.array([.229, .224, .225])
        mean = np.array([.485, .456, .406])
        out_img = (np_input_img * std + mean) * 255
...
def replace_lbl_id(np_img, before_id, after_id):
    idx = np.where(np_img == before_id)
    np_img[idx] = after_id
    return np_img


def get_img_transform(img_shape, normalize_way="imagenet", use_crop=False):
    img_transform_list = [] if use_crop else [Scale(img_shape, Image.BILINEAR)]
    img_transform_list.append(ToTensor())

    if normalize_way == "imagenet":
        img_transform_list.append(Normalize([.485, .456, .406, .485, .485, .485], [.229, .224, .225, .229, .229, .229]))
        # img_transform_list.append(Normalize([.485, .456, .406], [.229, .224, .225]))
        print ("ImageNet Normalization!")
    elif normalize_way == "city":
        img_transform_list.append(Normalize([0.290101, 0.328081, 0.286964], [0.182954, 0.186566, 0.184475]))

    else:
        print ("No normalization...")

    return Compose(img_transform_list)


def get_lbl_transform(img_shape, n_class, background_id=255, use_crop=False):
    label_transform = [] if use_crop else [Scale(img_shape, Image.NEAREST)]
    label_transform += [
        ToLabel(),
        ReLabel(background_id, n_class - 1),
    ]
    return Compose(label_transform)
